...
import { canHandoff, draftFor, eligibleForBulkEmail, preferredRoute } from "./model";
...
describe("신규 기업 탐색과 컨택 경계", () => {
  ...
  const emailOnlyCompany = {
    ...linkedInCompany,
    people: [{ id: "email-person", name: "담당자", role: "Manager", email: "manager@example.com" }],
  };
  const task: ContactTask = {
    companyId: emailOnlyCompany.id, quarter: "2026-Q4", batchId: "initial",
    status: "ready", ...draftFor(emailOnlyCompany), personId: "email-person", channel: "email", sent: null,
  };
  ...
  it("이메일 일괄 발송은 이메일만 확보된 준비된 작업에 한정한다", () => {
    ...
    expect(eligibleForBulkEmail({ ...task, status: "pending" }, emailOnlyCompany)).toBe(false);
    expect(eligibleForBulkEmail({ ...task, sent: { channel: "email", recipient: "담당자", subject: "제목", body: "본문", at: "지금" } }, emailOnlyCompany)).toBe(false);
  });
});
