...
// 모든 POST/PUT/PATCH 관리자 변경에 적용한다. 같은 키로 재시도가 오면 이전에
// 성공한 응답을 그대로 재반환한다. 같은 키에 다른 payload가 오면 409. 업무
// 쓰기 + 키 저장을 한 트랜잭션으로 묶어서, 쓰기 성공 후 키 저장 전에 죽어도
// 재시도가 중복 쓰기를 만들지 않게 한다.
