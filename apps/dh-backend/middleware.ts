...
// 로컬 프론트(5173)와 운영 admin.ghsnu.com만 허용한다. 필요해지면
// ALLOWED_ORIGINS(콤마 구분)로 배포 프리뷰 URL 등을 추가할 수 있다.
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS ?? "http://localhost:5173,https://admin.ghsnu.com")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
function corsHeaders(origin: string | null): Headers {
  ...
  // Idempotency-Key는 우리 쓰기 API가 쓰는 커스텀 헤더라 기본 허용 목록엔 없다.
