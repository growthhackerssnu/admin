...
// #20 PUT /outreaches/{id}/recipient — 수신자 선택. 프론트가 이미 걸러 보여준
// 후보라도 서버가 다시 제외 대상인지 검증한다(#07과 같은 규칙).
export const PUT = withApiHandler<{ id: string }>(async (req, { member, params, requestId }) => {
  ...
  const result = await withIdempotency(req, member, "PUT /outreaches/:id/recipient", parsed.data, async (tx) => {
    ...
    const endpoint = await tx.contactEndpoint.findUnique({ where: { id: endpointId } });
    if (!endpoint || endpoint.contactId !== contactId || endpoint.companyId !== current.companyId) {
      throw new ApiError("VALIDATION_ERROR", "연락처 정보가 올바르지 않습니다.");
    }
