...
export const POST = withListupApiHandler<{ taskId: string }>(
  async (_req, { member, params }) => {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const isAutomaticContactTask =
        ...
        (task.type === "contact_research" ||
          task.type === "contact_verification");
      if (
        isAutomaticContactTask &&
        task.candidate &&
        task.candidate.effectiveFit !== "fit"
      ) {
        throw new ApiError(
          "TASK_NOT_RETRYABLE",
          "적합이 아닌 후보의 자동 연락 조사는 재개하지 않습니다.",
          {
            fieldErrors: {
              effectiveFit: task.candidate.effectiveFit ?? "not_assessed",
            },
          },
        );
      }
    ...
    if (
      updated.type === "company_discovery" ||
      updated.type === "company_research" ||
      updated.type === "fit_assessment" ||
      updated.type === "contact_research"
    )
      await notifyWorker(updated.id);
