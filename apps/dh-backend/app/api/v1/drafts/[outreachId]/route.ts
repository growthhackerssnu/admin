...
// #23/#24 GET·PATCH /drafts/{outreachId}
// 우리 스키마엔 outreach 하나당 초안 스레드가 하나뿐이라(05 문서 §3 "컨택 건은
// 기업당 하나로 이어진다"), api-contract.md가 말하는 "draftId"는 outreachId로
// 쓴다 — message_draft_revisions 개별 행 id는 리비전 이력일 뿐 클라이언트가
// 안정적으로 들고 있을 식별자가 아니다.

export const GET = withApiHandler<{ outreachId: string }>(async (_req, { params, requestId }) => {
...
// 다시 검토해야 한다.
export const PATCH = withApiHandler<{ outreachId: string }>(async (req, { member, params, requestId }) => {
  const body = await req.json().catch(() => null);
  const parsed = saveDraftSchema.safeParse(body);
  if (!parsed.success) throw new ApiError("VALIDATION_ERROR", "입력값을 확인하세요.");
  const { expectedVersion, expectedRevision, topic, subject, body: draftBody } = parsed.data;

  const result = await withIdempotency(req, member, "PATCH /drafts/:outreachId", parsed.data, async (tx) => {
