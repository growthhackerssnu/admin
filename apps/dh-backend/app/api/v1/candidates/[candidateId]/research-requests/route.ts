  ...
  async (req, { member, params }) => {
    ...
    const result = await withIdempotency(
      ...
      async (tx) => {
        ...
        const isContactWork =
          input.type === "contact_research" ||
          input.type === "contact_verification";
        if (isContactWork && candidate.effectiveFit !== "fit") {
          throw new ApiError(
            "FIT_REQUIRED",
            "적합으로 판단된 후보만 연락 조사를 요청할 수 있습니다.",
            {
              fieldErrors: {
                effectiveFit: candidate.effectiveFit ?? "not_assessed",
              },
            },
          );
        }
        ...
        if (input.type === "contact_verification") {
          const existing = await tx.contactOptionAssessment.count({
    ...
    const taskId = (result.body as { data: { task: { id: string } } }).data.task
      .id;
    if (input.type === "company_research" || input.type === "contact_research")
