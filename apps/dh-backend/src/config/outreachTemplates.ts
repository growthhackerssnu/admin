...
export const NEW_OUTREACH_TEMPLATES = (["email", "linkedin"] as const).map(
  (channel) => ({
    id: `new-outreach-${channel}-v${NEW_OUTREACH_TEMPLATE_VERSION}`,
    route: "new" as const,
    channel,
    version: NEW_OUTREACH_TEMPLATE_VERSION,
    subjectTemplate,
    bodyTemplate,
  }),
);
